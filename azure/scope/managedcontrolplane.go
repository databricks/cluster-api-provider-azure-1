...
func NewManagedControlPlaneScope(ctx context.Context, params ManagedControlPlaneScopeParams) (*ManagedControlPlaneScope, error) {
	...
	return &ManagedControlPlaneScope{
		...
		cache:                  params.Cache,
		DisableSubnetReconcile: params.DisableSubnetReconcile,
		DisableVnetReconcile:   params.DisableVnetReconcile,
	}, nil
}
...
func (s *ManagedControlPlaneScope) IsNotPaused() bool {
...
func (s *ManagedControlPlaneScope) ManagedClusterSpec() (azure.ManagedClusterSpec, error) {
	...
	if s.ControlPlane.Spec.AutoScalerProfile != nil {
		managedClusterSpec.AutoScalerProfile = &azure.AutoScalerProfile{
		...
		// If the AutoScalerProfile is nil in the AzureManagedControlPlane CR, then the defaults are set.
		// This is so the desired state does not generate unnecessary diffs when the current state is using defaults,
		// and also so that a desired state of nil correctly sets the defaults.
		managedClusterSpec.AutoScalerProfile = &azure.AutoScalerProfile{
			ScaleDownDelayAfterDelete:     to.StringPtr("10s"),
			ScaleDownUtilizationThreshold: to.StringPtr("0.5"),
			SkipNodesWithSystemPods:       to.StringPtr("true"),
		}
	}
...
func buildAgentPoolSpec(managedControlPlane *infrav1exp.AzureManagedControlPlane,
	...
	// Handle EnableAutoScaling:
	// - If explicitly set, use the provided value
	// - If not set but Scaling is provided, default to true (backward compatibility)
	if managedMachinePool.Spec.EnableAutoScaling != nil {
		agentPoolSpec.EnableAutoScaling = managedMachinePool.Spec.EnableAutoScaling
		if *managedMachinePool.Spec.EnableAutoScaling && managedMachinePool.Spec.Scaling != nil {
	...
	} else if managedMachinePool.Spec.Scaling != nil {
		// Backward compatibility: if Scaling is provided without explicit EnableAutoScaling, enable it
