...
func (s *ManagedControlPlaneScope) GetAgentPoolSpecs(ctx context.Context) ([]azure.AgentPoolSpec, error) {
	...
	for _, pool := range s.AllNodePools {
		...
		if pool.Spec.AdditionalTags != nil {
			ammp.AdditionalTags = map[string]*string{}
			for k, v := range pool.Spec.AdditionalTags {
				val := v
				ammp.AdditionalTags[k] = &val
			}
		}
...
func (s *ManagedControlPlaneScope) AgentPoolSpec() azure.AgentPoolSpec {
	...
	if s.InfraMachinePool.Spec.AdditionalTags != nil {
		agentPoolSpec.AdditionalTags = map[string]*string{}
		for k, v := range s.InfraMachinePool.Spec.AdditionalTags {
			val := v
