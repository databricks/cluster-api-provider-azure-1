...
func (m *AzureManagedControlPlane) validateAutoScalerProfile() error {
	autoScalerProfile := m.Spec.AutoScalerProfile
	if autoScalerProfile != nil {
		var allErrs field.ErrorList

		if autoScalerProfile.ScaleDownDelayAfterDelete != nil {
			if ptr.Deref(autoScalerProfile.ScaleDownDelayAfterDelete, "") != "" {
				if !rScaleDownDelayAfterDelete.MatchString(ptr.Deref(autoScalerProfile.ScaleDownDelayAfterDelete, "")) {
					allErrs = append(allErrs, field.Invalid(field.NewPath("spec", "autoscalerProfile", "scaleDownDelayAfterDelete"), autoScalerProfile.ScaleDownDelayAfterDelete, "invalid value"))
				}
			}
		}
